...
This Claude instance works in `~/push-films-website/` only.
The finance spreadsheet lives in a separate folder (`CLAUDE VERSION - MARCH 26/`) and has its own Claude instance. Do not read, edit, or run scripts in that folder. The only permitted interaction with the finance project is running `generate_dashboard.py` (which lives here) to regenerate `dashboard_data.json`.
...
- **Financial dashboard**: `financial-dashboard.html` — personal finance dashboard for Greg. Loads all data from `dashboard_data.json` at runtime (no hardcoded figures). Live at `greghobden.com/financial-dashboard.html`.
- **Dashboard data**: `dashboard_data.json` — generated by `generate_dashboard.py`. Contains `monthly`, `cats`, `atl` arrays. Never edit this by hand — regenerate it from the spreadsheet.
- **Dashboard generator**: `generate_dashboard.py` — reads the most recently modified `ACCOUNTS*.xlsx` from the iCloud finance folder, writes `dashboard_data.json`. Run with `/opt/homebrew/bin/python3 generate_dashboard.py` from the push-films-website folder. After running: `git add dashboard_data.json && git commit -m "Update dashboard data" && git push`.

## Financial Dashboard

### Architecture
- `financial-dashboard.html` fetches `dashboard_data.json` on load via `fetch()` — requires being served over HTTP (Netlify or local server), not opened as a `file://` URL
- All chart rendering, KPI boxes, and income/spend breakdowns are driven entirely by the JSON
- `generate_dashboard.py` auto-detects the latest spreadsheet, reads income from monthly tabs + spending from SUMMARY sheet, preserves manually annotated `from`/`breakdown`/`tax` fields from the existing JSON

### Data Update Workflow
1. Spreadsheet updated by Claude Code instance (separate session)
2. Run: `/opt/homebrew/bin/python3 generate_dashboard.py`
3. Run: `git add dashboard_data.json && git commit -m "Update dashboard data" && git push`
4. Netlify deploys automatically — dashboard live within ~60 seconds
...
```json
{
  ...
  "monthly": [{"m":"SEP 2024","spend":0,"running":0,"vs":0}],
  "cats":    [{"m":"SEP 2024","Food & Drink":0,"Transport":0,"Comms":0,"Home":0,"Finance/Work":0,"Lifestyle":0,"Personal":0,"Family":0}],
  "atl":     [{"m":"SEP 2024","income":0,"costs":0,"tax":0,"from":"","breakdown":[{"p":"Company","a":0}]}]
}
```

### Key Dashboard Facts
- Tax years: Sep–Aug (TY24/25 = Sep 2024 – Aug 2025)
- Calendar years: Jan–Dec (CY2025 = Jan–Dec 2025; CY2024 = Sep–Dec 2024 as data starts Sep 2024)
- Ex-VAT income = gross / 1.2 (Greg is VAT registered)
- Taxable profit = ex-VAT income − project costs; estimated tax = 23%
